    ...
    internal class WorkflowTestHost : IDisposable
    {
        ...
        protected void StartFunctionRuntime(WorkflowTestInput[] inputs, string localSettings, string parameters, string connectionDetails, string host, DirectoryInfo artifactsDirectory)
        {
            try
            {
                ...
                // Wait for process to fully start-up and be ready for reading http requests
                WaitForProcessStartupCompleted();
        ...
        private static void WaitForProcessStartupCompleted()
        {
            var httpClient = new HttpClient();
            var startTime = DateTime.Now;
            while ((DateTime.Now - startTime).Milliseconds < 5000)
            {
                try
                {
                    var response = httpClient.GetStringAsync(TestEnvironment.FlowV2TestHostUri).Result;
                    if (!string.IsNullOrEmpty(response) && response.Contains("app is up and running"))
                    {
                        break;
                    }
                }
                ...
                {
                    // Swallow 'connection refused' etc.
                }
